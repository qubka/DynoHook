...
void* Trampoline::AllocateTrampoline(void* sourceAddress, bool& restrictedRelocation) {
    ...
    int32_t pageSize = (int32_t) Memory::GetPageSize();
    ...
    while (!trampoline) {
        ...
        int64_t targetAddress = (int64_t)sourceAddress + signedIntMaxValue + 5 - (++allocationAttempts * pageSize);
#elif DYNO_ARCH_X86 == 32
...
void* Trampoline::AllocateTrampolineWithinBounds(void* sourceAddress, int64_t lowestRipRelativeMemoryAccess, int64_t highestRipRelativeMemoryAddress, bool& restrictedRelocation) {
    ...
    while (!trampoline) {
        ...
        int64_t targetAddress = (int64_t)initialTargetAddress - trampolineLengthUpperBound - (++allocationAttempts * pageSize);
