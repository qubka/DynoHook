...
TEST_CASE("Test Disassemblers x64", "[ZydisDisassembler]") {
	...
	uintptr_t PrevInstAddress = (uintptr_t)&x64ASM.front();
	...
	SECTION("Check disassembler integrity") {
		...
		for (const auto& [dest, inst] : disasm.getBranchMap()) {
			std::cout << std::hex << "dest: " << dest << " " << std::dec << inst << std::endl;
		}
	...
	SECTION("Check instruction re-encoding integrity") {
        ...
        accessor.writeEncoding(Instructions[8]);

		Instructions[9].setRelativeDisplacement(0x00);
        accessor.writeEncoding(Instructions[9]);
		...
		// undo writes
		x64ASM = vecCopy;
		Instructions = disasm.disassemble((uintptr_t)&x64ASM.front(), (uintptr_t)&x64ASM.front(),
			(uintptr_t)&x64ASM.front() + x64ASM.size(), accessor);
	}
	...
	SECTION("Check multiple calls") {
		dyno::insts_t insts;
		for (int i = 0; i < 100; i++) {
			insts = disasm.disassemble((uintptr_t)&x64ASM.front(), (uintptr_t)&x64ASM.front(),
				(uintptr_t)&x64ASM.front() + x64ASM.size(), accessor);
		}
	}

	SECTION("Verify branching, relative fields") {
		dyno::insts_t insts = disasm.disassemble((uintptr_t)&x64ASM.front(), (uintptr_t)&x64ASM.front(),
			(uintptr_t)&x64ASM.front() + x64ASM.size(), accessor);
	...
	SECTION("Test garbage instructions") {
		char randomBuf[500];
        for (char& i : randomBuf)
            i = static_cast<char>(randByte());

		auto insts = disasm.disassemble((uintptr_t)randomBuf, (uintptr_t)0x0,
										500, accessor);
...
TEST_CASE("Test Disassemblers x86", "[ZydisDisassembler]") {
	...
	uintptr_t PrevInstAddress = (uintptr_t)&x86ASM.front();
	size_t   PrevInstSize = 0;
	...
	SECTION("Check instruction re-encoding integrity") {
        ...
        accessor.writeEncoding(Instructions.at(3));

		Instructions.at(6).setRelativeDisplacement(0x00);
        accessor.writeEncoding(Instructions.at(6));
		...
		x86ASM = vecCopy;
		Instructions =
			disasm.disassemble((uintptr_t)&x86ASM.front(), (uintptr_t)&x86ASM.front(),
			(uintptr_t)&x86ASM.front() + x86ASM.size(), accessor);
	}
	...
	SECTION("Check multiple calls") {
		dyno::insts_t insts;
		for (int i = 0; i < 100; i++) {
			insts = disasm.disassemble((uintptr_t)&x86ASM.front(), (uintptr_t)&x86ASM.front(),
				(uintptr_t)&x86ASM.front() + x86ASM.size(), accessor);
		}
	}

	SECTION("Verify branching, relative fields") {
		dyno::insts_t insts = disasm.disassemble((uintptr_t)&x86ASM.front(), (uintptr_t)&x86ASM.front(),
			(uintptr_t)&x86ASM.front() + x86ASM.size(), accessor);
...
TEST_CASE("Test Disassemblers x64 Two", "[ZydisDisassembler]") {
	...
	Instructions = disasm.disassemble((uintptr_t)&x64ASM3.front(), (uintptr_t)&x64ASM3.front(),
		(uintptr_t)&x64ASM3.front() + x64ASM3.size(), accessor);
...
TEST_CASE("Test Disassemblers NOPS", "[ZydisDisassembler]") {
	...
	SECTION("Verify multi-byte nops decodings x64") {
		for (auto& ins : Instructions) {
			...
			REQUIRE(dyno::ZydisDisassembler::isPadBytes(ins));
		}
	}
